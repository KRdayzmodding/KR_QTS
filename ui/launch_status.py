...
class LaunchStatus:
    ...
    def detach(self) -> None:
        """Отпускает блок прошлого прогона: он становится историей.

        Пока блок «наш», такт состояния переписывает его на месте. Начался
        новый запуск — старый блок трогать нельзя, иначе, сорвись подготовка,
        состояние нового запуска переписало бы строки предыдущего.
        """
        self._block = -1
