...
from PySide6.QtWidgets import (
    QVBoxLayout, QHBoxLayout, QGridLayout, QListWidgetItem, QColorDialog,
    QScrollArea, QWidget,
)
...
class FlagEditorDialog(ThemedDialog):
    ...
    def __init__(self, flag: ModFlagDef | None, parent=None):
        ...
        self.setWindowTitle(tr("flags.rename_title", "Флаг") if flag
                           else tr("flags.add_title", "Новый флаг"))
        self.resize(360, 260)
        layout = QVBoxLayout(self)
    ...
    def result_values(self) -> tuple[str, str, bool, bool, bool, str]:
        return (self.name_edit.text().strip(), self._color, self.chk_bold.isChecked(),
                self.chk_italic.isChecked(), self.chk_underline.isChecked(), self._icon)
...
class ModFlagsDialog(ThemedDialog):
    ...
    def __init__(self, registry: ModRegistry, parent=None):
        ...
        self.resize(360, 420)
        layout = QVBoxLayout(self)
        hint = CaptionLabel(tr("flags.hint",
    ...
    def _add(self) -> None:
        ...
        if not name:
            return
        self.defs.append(ModFlagDef(id=self._unique_id(_slug(name)), name=name, color=color,
                                    bold=bold, italic=italic, underline=underline, icon=icon))
...
class FlagAssignDialog(ThemedDialog):
    ...
    def __init__(self, mods: list[ModInfo], flag_defs: list[ModFlagDef], parent=None):
        ...
        for d in flag_defs:
            ...
            item.setCheckState(Qt.CheckState.Unchecked if n == 0
                               else Qt.CheckState.Checked if n == len(mods)
