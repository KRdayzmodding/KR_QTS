...
def link_color() -> str:
    ...
    Синий по умолчанию от Qt на тёмном фоне почти неразличим. Одного цвета на
    оба фона не хватает: лучший из общих даёт 3.4 к одному на белом при норме
    4.5 для текста. Но фон здесь наш собственный и известен заранее — в отличие
    от панели задач, — так что подбираем под тему. Оба тона того же оранжевого
    семейства, что и акцент приложения, и оба дают 5.3 к одному.
    """
    return "#e08a3c" if isDarkTheme() else "#a3571a"
