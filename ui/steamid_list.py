...
from qfluentwidgets import (
    BodyLabel, CaptionLabel, FluentIcon as FIF, LineEdit, ListWidget,
...
class SteamIdDialog(ThemedDialog):
    ...
    def __init__(self, api_key: str = "", current: str = "",
                 taken: tuple[str, ...] = (), parent: QWidget | None = None):
        super().__init__(parent)
        self.api_key = api_key
        ...
        layout.addWidget(BodyLabel(tr(
            "sid.dlg_head", "SteamID64 или ссылка на профиль Steam")))
    ...
    def _recheck(self) -> None:
        ...
        if kind == "id" and val in self.taken:
            ...
            self.hint.setText(tr("sid.vanity",
                                 "Имя профиля «{n}» — определим через Steam, "
                                 "нужна сеть.", n=val))
            self.b_ok.setEnabled(True)
        elif raw.isdigit():
            # самый частый промах — сказать сразу, чего именно не хватает
            self.hint.setText(tr("sid.digits",
                                 "SteamID64 состоит из 17 цифр, а здесь {n}.",
                                 n=len(raw)))
            self.b_ok.setEnabled(False)
        else:
            self.hint.setText(tr("sid.bad",
                                 "Не похоже ни на SteamID64 из 17 цифр, ни на "
                                 "ссылку вида steamcommunity.com/id/<имя>."))
            self.b_ok.setEnabled(False)
    ...
    def _accept(self) -> None:
        ...
        self._worker = ResolveSteamIdWorker(self.edit.text().strip(),
                                            self.api_key, self)
    ...
    def _resolved(self, sid: str, _source: str) -> None:
        ...
        if not sid:
            self.hint.setText(tr("sid.failed",
                                 "Steam не ответил или такого профиля нет. "
                                 "Проверьте ссылку или введите SteamID64."))
            self.b_ok.setEnabled(True)
            return
...
class SteamIdList(QWidget):
    ...
    def __init__(self, ids: list[str], api_key: Callable[[], str] | str = "",
