...
from PySide6.QtWidgets import (
    QVBoxLayout, QHBoxLayout, QTreeWidgetItem, QHeaderView, QInputDialog,
    QScrollArea, QWidget,
)
from qfluentwidgets import (
    PushButton, PrimaryPushButton, TreeWidget, ComboBox,
    SearchLineEdit, CaptionLabel, BodyLabel, HyperlinkLabel, InfoBar, InfoBarPosition,
    FluentIcon as FIF,
)
...
class ConnectModsDialog(ThemedDialog):
    ...
    def __init__(self, registry: ModRegistry, preset: ServerPreset,
        ...
        b_collection = PushButton(FIF.CLOUD_DOWNLOAD, tr("collection.connect_btn",
        ...
        self.tree.setHeaderLabels([
            tr("mods.col_name", "Мод"), tr("connect.col_type", "Тип"),
        ])
        ...
        hint = CaptionLabel(tr("connect.hint",
    ...
    def _make_item(self, mod: ModInfo, flag_defs: dict) -> _SortableItem:
        p = self.preset
        enabled = self.registry.index_of(mod, p.mods) is not None \
            or self.registry.index_of(mod, p.server_mods) is not None

        type_text = tr("connect.type_server", "Сервер") if mod.is_server \
            else tr("connect.type_client", "Клиент")
        ...
        if flag_ids:
            ...
            item.setToolTip(COL_NAME, tr("mods.flags_tip", "Флаги: {f}",
                                        f=", ".join(d.name for d in defs)))
        item.setToolTip(COL_TYPE, tr("connect.type_tip",
                                     "Клиент/Сервер задаётся признаком «Серверный» на вкладке «Моды»."))
        return item
    ...
    def _item_changed(self, item: QTreeWidgetItem, column: int) -> None:
        ...
        if not mod:
            return
        p = self.preset
        was_enabled = self.registry.index_of(mod, p.mods) is not None \
            or self.registry.index_of(mod, p.server_mods) is not None
