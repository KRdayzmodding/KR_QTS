...
class SteamWatcher(QObject):
    ...
    def __init__(self, parent=None, interval: int = POLL_MS):
        ...
        self._apps: dict[str, tuple] = {}    # ключ -> снимок для сравнения
    ...
    def _poll_apps(self, initial: bool) -> None:
        ...
        for appid, key in wanted.items():
            ...
            if was == "?" or was is None or not (was[0] & steam_state.STATE_FULLY_INSTALLED):
    ...
    def states(self) -> dict:
        ...
        """
        wanted = {SETTINGS_APPS[k][0]: k for k in self._keys}
        return {wanted[appid]: st
                for appid, st in steam_state.app_states(wanted).items()}
