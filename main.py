...
def main() -> int:
    ...
    first_run = not settings.first_run_done     # мастер переключит флаг сам
    if first_run:
    ...
    # Главное окно собирается за спиной проверки обновлений: проверка ждёт
    # сеть, сборка занимает процессор — по очереди это лишняя секунда с
    # лишним. Пока оно заготовка (live=False), о себе оно не заявляет.
    def build_window():
        return MainWindow(settings, live=False)

    if first_run or quiet or wanted:
        # Подняли по ярлыку или из qtsctl — вопросов про версию не задаём:
        # там ждут запуска сервера. На первом запуске проверка уже была, до
        # мастера.
        window = build_window()
    else:
        # Обновляться удобнее, пока работа не начата: в начатую работу с этим
        # уже не лезем, там обновление живёт пометкой в панели разделов. Окна
        # не будет, если проверка выключена в настройках; запереть нас оно не
        # может ни при каких обстоятельствах.
        from ui.update_gate import run as update_gate
        go, window = update_gate(settings, build=build_window)
        if not go:
            return 0                # человек согласился обновиться, мы уходим

    window.go_live()                # значок в трее и такт состояния
