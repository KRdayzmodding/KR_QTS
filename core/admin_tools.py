...
def detect_tools(mods: list[ModInfo]) -> list[AdminTool]:
    ...
    for tool in KNOWN_TOOLS:
        for mod in mods:
            ...
            if mod.workshop_id in tool.workshop_ids \
                    or any(mark in folder for mark in tool.folder_markers):
                found.append(tool)
                break
    return found
...
def _apply_lb(profile: Path, steamids: list[str]) -> list[str]:
    ...
    existing = {str(a.get("steamid", "")).strip()
...
def _apply_tool(tool: AdminTool, profile: Path, steamids: list[str],
...
def apply(profile_dir: str | Path, mods: list[ModInfo] | None,
         steamids: list[str], password: str = "") -> list[tuple[AdminTool, list[str]]]:
