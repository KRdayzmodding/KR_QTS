...
def _format(exc_type: type[BaseException], exc: BaseException,
            tb: TracebackType | None, where: str) -> tuple[str, str]:
...
def _report(exc_type: type[BaseException], exc: BaseException,
            tb: TracebackType | None, where: str) -> None:
...
def install(app_label: str = "", log_dir: Path | None = None) -> None:
    ...
    threading.excepthook = lambda args: _report(
        args.exc_type, args.exc_value or args.exc_type(),
        args.exc_traceback, getattr(args.thread, "name", "фоновый"))
