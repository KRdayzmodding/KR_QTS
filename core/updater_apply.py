...
def preflight(rel: Release | None = None) -> str:
    ...
    if not _writable(target):
        return (f"нет прав на запись в {target} — "
...
class ExtractWorker(QThread):
    ...
    def __init__(self, rel: Release, exe: str,
...
def write_helper(src: Path, dst: Path, exe: str, *,
                 title: str = "KR Quick Test Server",
    ...
    text = _SCRIPT.format(
        title=title, message=message, stuck=stuck, failed=failed,
        pid=pid if pid is not None else os.getpid(),
        tries=_WAIT_TRIES,
        src=src, dst=dst, exe=exe, staging=STAGING, excl=excl,
    )
...
def launch_helper(script: Path) -> None:
    ...
    """
    subprocess.Popen(f'cmd.exe /c ""{script}""',
