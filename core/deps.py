...
def resolve(roots: list[ModInfo], registry: ModRegistry, api_key: str = "",
            max_depth: int = MAX_DEPTH) -> DepResult:
    ...
    for _ in range(max_depth):
        ...
        for mod in frontier:
            for kind, ident in _direct_deps(mod, api_key):
                dep = by_workshop.get(ident) if kind == "steam" \
                    else registry.mods.get(ident.lower())
                if dep is not None:
                    key = mod_key(dep)
                    if key in seen_mods:
...
def filter_connected(res: DepResult, registry: ModRegistry,
                     mods: list[str], server_mods: list[str]) -> DepResult:
    ...
    """
    return DepResult(
        found=[m for m in res.found
               if registry.index_of(m, mods) is None
               and registry.index_of(m, server_mods) is None],
