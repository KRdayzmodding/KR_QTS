...
Filepatching позволяет подтягивать скрипты без запаковки PBO — достаточно
...
def _create(link: Path, target: Path) -> tuple[str, str]:
    ...
    if _is_link(link):
        ...
        return "failed", tr("filepatch.err_real_dir",
...
def add(settings: Settings, p_path: str | Path) -> tuple[Report, str]:
    ...
    if not is_on_p_drive(path):
        return Report(), tr("filepatch.err_not_p",
                            "Папка должна находиться на диске P:. Выбрано: {p}", p=path)
    if not rel_parts(path):
        return Report(), tr("filepatch.err_root",
                            "Нельзя подключить корень диска P: — укажите папку скриптов мода.")
    if not path.is_dir():
    ...
    if not game_roots(settings):
        return Report(), tr("filepatch.err_no_roots",
                            "Не задан ни один путь к клиенту или серверу — некуда ставить ссылки.")
...
def remove_all(settings: Settings) -> Report:
    ...
    функцией: моды (KR_Debug/MODS) и прочее содержимое каталогов не затрагивается.
    """
    rep = Report()
...
def status_lines(settings: Settings) -> list[str]:
    ...
    for entry in settings.filepatch_links:
        ...
        lines.append(tr("filepatch.line", "{p} — ссылок: {ok} из {total}{gone}",
                        p=entry, ok=ok, total=len(roots), gone=gone))
    return lines
